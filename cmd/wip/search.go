...
var searchCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if fromStr != "" {
			...
			// Default start if not specified?
			// Maybe 1 month ago? Or beginning of time?
			// Let's say 30 days ago default if query is present, otherwise today?
			// If user searches "bug", they probably mean anytime.
		...
		// Sort Newest -> Oldest for search results?
		// Usually search results are relevant. Chronological is fine.
		// Let's sort Oldest -> Newest (same as GetEvents returns usually, but let's ensure)
