...
func RunCapture(s store.Store, eventType string, gitShow GitShowFunc) error {
	...
	// 3. Gather Context (same as root command)
	// We can reuse logic or extract it. For now duplicating minimal needed.
	ctx := model.Context{}
	...
	event := &model.WipsEvent{
		...
		Content: strings.TrimSpace(content), // Store full git show output as content?
		// Spec says "Commit Msg, Hash, Diff Stats"
		// git show default output contains all these.
		Ctx: ctx,
	}
